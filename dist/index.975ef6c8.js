...
async function updateBreedSelect() {
    ...
    try {
        ...
        breedSelect.innerHTML = breeds.map((breed)=>`<option value="${breed.id}">${breed.name}</option>`).join("") || "<option disabled>No breeds available</option>";
    } catch  {
        toggleError(true);
    } finally{
        toggleLoader(false);
...
async function displayCatInfo(breedId) {
    ...
    try {
        ...
        if (!cat || !cat.breeds || cat.breeds.length === 0) throw new Error("Invalid cat data");
...
breedSelect.addEventListener("change", (event)=>{
    ...
    if (breedId) displayCatInfo(breedId); // Afișează informațiile pisicii selectate
});
// Inițializează aplicația
updateBreedSelect();

},{"./cat-api.js":"d7YdZ","./style.css":"bhJkM"}],"d7YdZ":[function(require,module,exports) {
...
(0, _axiosDefault.default).defaults.headers.common["x-api-key"] = "live_Gh72RCWOJulaAMZVxMGvGKIJSZ0vxofLdrKM7cOT3EPpN62UfhLTU30cVso9eb5r";
...
async function fetchCatByBreed(breedId) {
    try {
        ...
        if (response.data.length === 0) throw new Error("No cat found for this breed");
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"bhJkM":[function() {},{}]},["1RB6v","8lqZg"], "8lqZg", "parcelRequired7c6")
